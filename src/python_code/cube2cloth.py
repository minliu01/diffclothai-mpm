...
def lossFunction(xvPairs):
    target = torch.full((x0_torch.shape[0]//3, ), 3.0)
    targetLast = 50*torch.nn.functional.smooth_l1_loss(xvPairs[-1][0].reshape(-1,3)[:,1], target)
    return targetLast
...
sim.primitives[0].isInCollision = False
sim.primitives[1].isInCollision = True
sim.primitives[2].isInCollision = True
