...
void runBackwardTask(int demoIdx, bool isRandom, int srandSeed) {
  ...
  BackwardTaskSolver::solveDemo(clothSystem, [&](const std::string &v) {}, demoIdx, isRandom, srandSeed);
  delete clothSystem;
}
...
#pragma omp single
  ...
  std::vector<std::string> validDemos = {"tshirt", "sock", "hat", "sphere", "dress", "clothHanger"};
  ...
  if (argc == 1) {
    ...
    } else if (demoName == "clothHanger") {
      demo = Demos::DEMO_CLOTH_HANGER;
    }
     if (mode == "visualize") {
