...
def parseInputs():
    ...
    parser.add_argument('--time', action = 'store_true', default=False, help='report differnet stages running time')
    ...
    parser.add_argument('--randSeed', type=int, default=3407)
    parser.add_argument('--epochNum', type =int, default=200, help='Number of epochs to train')
    parser.add_argument('--load_epoch', type =str, default='trainBestEpoch', help='if evaluating or resuming training, the epoch of the experiment to load')
...
def getTorchVectors_hanger(x0, v0, fixedPointInitPoses, targetTranslation):
    toDouble = False
    fixedPointInitDistance = fixedPointInitPoses[3:6] - fixedPointInitPoses[0:3]
    clip_left = toTorchTensor(fixedPointInitPoses[0:3].copy(), False, toDouble)
    clip_right = toTorchTensor(fixedPointInitPoses[3:6].copy(), False, toDouble)
    targetTranslation = np.tile(targetTranslation, x0.shape[0] // 3)
    targetshape = x0 + targetTranslation
    x0_torch = toTorchTensor(x0, False, toDouble)
    v0_torch = toTorchTensor(v0, False, toDouble)
    a_torch = toTorchTensor(fixedPointInitPoses, True, toDouble)
    CLIP_REST_DIST = np.linalg.norm(fixedPointInitDistance, 2)
    targetshape_torch = toTorchTensor(targetshape, False, toDouble)
    return x0_torch, v0_torch, a_torch, targetshape_torch, CLIP_REST_DIST, clip_left, clip_right
