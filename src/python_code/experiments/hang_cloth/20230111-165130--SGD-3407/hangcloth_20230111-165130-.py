...
def lossFunction(xvPairs):
    ...
    loss = {'succeed':succeed, 'total':targetLast}
    print('[last-frame loss]:{:.5f}'.format(targetLast.item()))
    return loss
...
sim.primitives[0].centerInit = np.array([0., -3., 0.])
sim.resetSystem()
...
targetTranslation = np.array([-0.2,0.3,0.1])
