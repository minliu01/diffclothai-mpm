...
def plotLosses_hanger(trainLosses, path):
    epochs = range(1, len(trainLosses)+1)
...
def plotCubePos(cube_positions, path):
    epochs = range(1, len(cube_positions)+1)

    fig = plt.figure()

    plt.plot(epochs, cube_positions, 'r', label='Cube Positions')
    plt.title('Cube Positions')
    plt.xlabel('Epochs')
    plt.ylabel('Position')
    plt.legend()
    plt.savefig("{}/position.png".format(path))
