...
#include "ccd.cpp"
#include "interval_root_finder.cpp"
#include "avx.cpp"
...
Simulation::isInContactWithObstacle(const Vec3d &pos, const Vec3d &v_in, const VecXd &x_prim_new,
  ...
  for (int i = 0; i < primitives.size(); i++) {
    ...
    if (p->isEnabled) {
      // test t=0
      if (p->isInContact(primPos, pos, v_in, info.normal, info.dist, info.v_out, info.contact_point)) {
        info.collides = true;
        info.primitiveId = i;
      ...
      if (p->isInContact(primPos, pos + v_in * sceneConfig.timeStep * 0.5, v_in, info.normal, info.dist, info.v_out, info.contact_point)) {
        info.collides = true;
        info.primitiveId = i;
      ...
      // test t=timestep
      if (p->isInContact(primPos, pos + v_in * sceneConfig.timeStep, v_in, info.normal, info.dist, info.v_out, info.contact_point)) {
        info.collides = true;
        info.primitiveId = i;
...
Vec3d Simulation::calcualteDryFrictionForce(const Vec3d &n, const Vec3d &f_i, double mu, int& type) const {
  Vec3d r_i;
  r_i.setZero();
...
void Simulation::step() {
  ...
  std::map<int, std::set<int>> clipInfos;
...
#pragma omp parallel for if (OPENMP_ENABLED)
  ...
  returnRecord.clipInfos = clipInfos;
  ...
  returnRecord.M = sysMat[currentSysmatId].P - sysMat[currentSysmatId].C;
...
void Simulation::initScene() {
  ...
  switch (sceneConfig.primitiveConfig) {
    ...
    case CLOTH_HANGER: {
      ...
      centerHighPoint[1] = restShapeMaxDim[1];
      cube1m.mu = 0.8;
      cube1m.center = cube1m.centerInit = Vec3d(0., -3., -2.);
      break;
    }
    ...
    case SLOPE: {
      ...
      slope.center = slope.centerInit =
              referencePoint + shift + Vec3d(0, 2, -4);
      slope.mu = 0.2;
